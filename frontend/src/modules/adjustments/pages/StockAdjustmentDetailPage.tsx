...
function userLabel(
  u: { email: string | null; phone: string | null } | null | undefined,
): string {
  if (!u) return "—";
...
function ItemDialog({
  ...
  const [qtyChange, setQtyChange] = useState(
    initial ? String(initial.qtyChange) : "",
  );
  ...
  const noProducts =
    !!locationId && registeredProducts !== undefined && products.length === 0;
        ...
        <Box sx={{ display: "flex", flexDirection: "column", gap: 2, mt: 1 }}>
          ...
          {noProducts && (
            <Alert severity="warning">
              No active products at this location
            </Alert>
          )}
          <FormControl
            fullWidth
            size="small"
            disabled={!locationId || noProducts}
          >
...
export default function StockAdjustmentDetailPage() {
  ...
  const [confirmAction, setConfirmAction] = useState<
    "submit" | "approve" | "finalize" | "delete" | null
  >(null);
  ...
  const isManager =
    isAdmin || (myLocations ?? []).some((l) => l.role === "MANAGER");
  ...
  const itemLocationIds = [
    ...new Set((req?.items ?? []).map((i) => i.locationId)),
  ];
  ...
  const noManagersAtItemLocations =
    ...
    itemLocationReadinesses.every(
      (r: { hasManager: boolean }) => !r.hasManager,
    );
  ...
  const addItemMutation = useMutation({
    mutationFn: (payload: AddItemPayload) =>
      stockAdjustmentsService.addItem(id!, payload),
    onSuccess: () => {
      invalidate();
  ...
  const updateItemMutation = useMutation({
    ...
    onError: () =>
      setSnack({ msg: "Failed to update item", severity: "error" }),
  });

  const deleteItemMutation = useMutation({
    mutationFn: (itemId: string) =>
      stockAdjustmentsService.deleteItem(id!, itemId),
    onSuccess: () => {
      invalidate();
      setSnack({ msg: "Item removed", severity: "success" });
    },
    onError: () =>
      setSnack({ msg: "Failed to remove item", severity: "error" }),
  });
  ...
  const submitMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setSnack({ msg: "Submitted successfully", severity: "success" });
    },
    onError: (e: any) => {
      setConfirmAction(null);
      setSnack({
        msg: e?.response?.data?.error?.message ?? "Submit failed",
        severity: "error",
      });
    },
  });
  ...
  const approveMutation = useMutation({
    ...
    onSuccess: () => {
      invalidate();
      setConfirmAction(null);
      setSnack({ msg: "Approved", severity: "success" });
    },
    onError: (e: any) => {
      setConfirmAction(null);
      setSnack({
        msg: e?.response?.data?.error?.message ?? "Approve failed",
        severity: "error",
      });
    },
  });
  ...
  const rejectMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setSnack({ msg: "Rejected", severity: "success" });
    },
    onError: (e: any) => {
      setSnack({
        msg: e?.response?.data?.error?.message ?? "Reject failed",
        severity: "error",
      });
    },
  });
  ...
  const finalizeMutation = useMutation({
    ...
    onSuccess: () => {
      invalidate();
      setConfirmAction(null);
      setSnack({ msg: "Finalized — stock updated", severity: "success" });
    },
    onError: (e: any) => {
      setConfirmAction(null);
      setSnack({
        msg: e?.response?.data?.error?.message ?? "Finalize failed",
        severity: "error",
      });
    },
  });
  ...
  const cancelMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setSnack({ msg: "Request cancelled", severity: "success" });
    },
    onError: (e: any) => {
      setSnack({
        msg: e?.response?.data?.error?.message ?? "Cancel failed",
        severity: "error",
      });
    },
  });
  ...
  const deleteMutation = useMutation({
    ...
    onError: (e: any) => {
      setConfirmAction(null);
      setSnack({
        msg: e?.response?.data?.error?.message ?? "Delete failed",
        severity: "error",
      });
    },
  });
  ...
  const canCancel =
    !isDraft &&
    !isTerminal &&
    !approveRejectVisible &&
    (isAdmin || isCreator || isManager);

  if (isLoading) return <CircularProgress />;
  if (error || !req)
    return <Alert severity="error">Failed to load request</Alert>;

  return (
    <Box>
      {/* Header */}
      {(() => {
        const inactiveForFinalize = isApproved
          ? req.items.filter((i) => i.isActiveNow === false)
          : [];
        return (
          <>
            <Box
            ...
            {inactiveForFinalize.length > 0 && (
              <Alert severity="error" sx={{ mb: 2 }}>
                {inactiveForFinalize.length} item(s) have inactive product
                registrations and cannot be finalized. Reactivate or remove them
                first.
              </Alert>
            )}
            ...
            {noManagersAtItemLocations && !isTerminal && (
              <WorkflowWarningBanner
                message={WORKFLOW_WARNINGS.adjustmentMissingManagers}
              />
            )}
          </>
        );
        ...
        <Box sx={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
          ...
          {req.approvedAt && (
            <Box>
              ...
              <Typography>
                {userLabel(req.approvedBy)} — {fmtDate(req.approvedAt)}
              </Typography>
            </Box>
          )}
          {req.finalizedAt && (
            <Box>
              ...
              <Typography>
                {userLabel(req.finalizedBy)} — {fmtDate(req.finalizedAt)}
              </Typography>
            </Box>
          )}
          {req.rejectedAt && (
            <Box>
              ...
              <Typography>
                {userLabel(req.rejectedBy)} — {fmtDate(req.rejectedAt)}
              </Typography>
            </Box>
          )}
          {req.rejectionReason && (
          ...
          {req.cancelledAt && (
            <Box>
              ...
              <Typography>
                {userLabel(req.cancelledBy)} — {fmtDate(req.cancelledAt)}
              </Typography>
            </Box>
          )}
        ...
        <TableContainer>
          <Table size="small">
              ...
              <TableRow>
                ...
                <TableCell align="right">
                  <Tooltip
                    title={
                      isDraft
                        ? "Live available stock at this location (updates in real-time)"
                        : "Stock snapshot captured when item was added to the request"
                    }
                    arrow
                  >
                    <span
                ...
                <TableCell align="right">
                  <Tooltip
                    title={
                      isDraft
                        ? "Projected stock after this adjustment (live, recalculated)"
                        : "Projected stock after adjustment (from creation snapshot)"
                    }
                    arrow
                  >
                    <span
            ...
            <TableBody>
              ...
              {req.items.map((item) => {
                const afterQty =
                  item.afterQty != null ? Number(item.afterQty) : null;
          ...
          <Box sx={{ display: "flex", gap: 2, flexWrap: "wrap" }}>
            ...
            {isApproved && isManager && (
              <Button
                ...
                disabled={
                  req.items.length === 0 ||
                  req.items.some((i) => i.isActiveNow === false) ||
                  noManagersAtItemLocations
                }
            ...
            return (
              ...
              >
                {hasInactiveForApprove && (
                  <Alert severity="warning">
                    Warning:{" "}
                    {req.items.filter((i) => i.isActiveNow === false).length}{" "}
                    item(s) have inactive product registrations. Approval will
                    still proceed.
                  </Alert>
                )}
                {confirmAction === "finalize" && (
                  <Alert severity="warning">
                    This will apply stock changes permanently and cannot be
                    undone.
                  </Alert>
                )}
                {confirmAction === "delete" && (
                  <Alert severity="error">
                    This will permanently delete the DRAFT request and all its
                    items.
                  </Alert>
                )}
                {(confirmAction === "submit" ||
                  (confirmAction === "approve" && !hasInactiveForApprove)) && (
                  <Alert severity="info">
